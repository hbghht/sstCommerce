...
{
    public class FilterByColorCommand : IQueryCommand
    {
        ...
        public FilterByColorCommand(IProductRepository repository, IBus bus, EventBus eventBus, ILogger<SearchCommand> logger)
        {
        ...
        public async Task Execute(MessageEvent messageEvent)
        {
            ...
            if(filterEvent != null)
            {
                Result = await _repository.FilterProductByColor(filterEvent.Value);
